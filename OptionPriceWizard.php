 ...
 * @author     Ruud Walraven <ruud.walraven@gmail.com>
 */
class OptionPriceWizard extends Widget
{
	...
	public function validate()
	{
		...
		if (is_array($options))
		{
			foreach ($options as $key=>$option)
			{
				...
				$options[$key]['price'] = trim($option['price']);
  ...
  <thead>
    <tr>
      ...
      <th>'.$GLOBALS['TL_LANG']['MSC']['ow_price'].'</th>
      <th>&nbsp;</th>
      <th>&nbsp;</th>
      <th>&nbsp;</th>
    </tr>
  ...
  <tbody>';
    ...
    <tr>
      ...
      <td><input type="text" name="'.$this->strId.'['.$i.'][price]" id="'.$this->strId.'_label_'.$i.'" class="tl_text_3" tabindex="'.++$tabindex.'" value="'.specialchars($this->varValue[$i]['price']).'"></td>
